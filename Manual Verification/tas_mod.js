...
Game.registerMod("TAS Controller", {
    init: function() {
        ...
        document.addEventListener('keydown', function(e) {
            switch(e.key.toLowerCase()) {
                case ' ': // Spacebar - wait until can click again
                    e.preventDefault();
                    if (self.manualMode) {
                        self.waitUntilCanClick();
                    }
                    break;
                case 's': // S key - advance one millisecond
                    e.preventDefault();
                    if (self.manualMode) {
                        self.advanceOneMs();
                    }
        ...
        console.log("SPACE - Wait until can click again (skip throttle)");
        console.log("S - Advance one millisecond (wait)");
    ...
    waitUntilCanClick: function() {
        // Calculate how many ms until we can click again
        var msUntilCanClick = 20 - (this.timeMs - this.lastClickTimeMs);
        
        if (msUntilCanClick <= 0) {
            console.log("Time " + this.timeMs + "ms - Already able to click!");
            this.updateDisplay();
            return;
        }
        
        console.log("Time " + this.timeMs + "ms - Waiting " + msUntilCanClick + "ms until can click...");
        
        // Advance time by the required amount
        for (var i = 0; i < msUntilCanClick; i++) {
            this.advanceOneMs();
        }
        
        console.log("Time " + this.timeMs + "ms - Can click now!");
    },
...
console.log("Press SPACE to skip to next clickable moment, S to wait 1ms, C to click (instant, 20ms throttle).");
console.log("Press 1-9 to buy buildings (instant). The mod tracks time and cookies baked to verify BFS.");
