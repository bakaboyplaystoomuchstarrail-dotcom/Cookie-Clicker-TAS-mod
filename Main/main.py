    ...
    <script>
        ...
        var Game = {{
            ...
            updateUI: function() {{
                ...
                for (var name in this.Objects) {{ var obj=this.Objects[name]; var price=this.getPrice(name); var buildingDiv=document.getElementById('building_'+name); if (buildingDiv) buildingDiv.className='building'+(this.cookies>=price?'':' unaffordable'); var costSpan=document.getElementById('cost_'+name); if (costSpan) costSpan.textContent=Math.ceil(price); var ownedSpan=document.getElementById('owned_'+name); if (ownedSpan) ownedSpan.textContent=obj.amount; var info2=document.getElementById('info2_'+name); if (info2) {{ var totalCps=(obj.amount*obj.baseCps).toFixed(1); info2.textContent='Total CPS: '+totalCps; }} }}
            }},
        ...
        var TASController = {{
            ...
            advanceOneMs: function() {{ this.timeMs+=1; var currentFrame=Math.floor(this.timeMs/this.msPerFrame); if (currentFrame>this.lastProductionFrame && Game.cookiesPs>0) {{ var productionThisFrame=Game.cookiesPs/this.fps; Game.cookies+=productionThisFrame; Game.cookiesEarned+=productionThisFrame; this.cookiesBaked+=productionThisFrame; this.lastProductionFrame=currentFrame; for (var n in Game.Objects) {{ var o=Game.Objects[n]; if (o.amount>0) o.produced += (o.amount*o.baseCps)/this.fps; }} }} Game.updateUI(); this.updateDisplay(); }},
            ...
            startAutomatedPlayback: function() {{ if (!this.automatedPath||this.automatedPath.length===0) {{ this.log('ERROR: No path loaded!','error'); return; }} document.getElementById('tasStatus').textContent='Status: Running...'; var msg=document.getElementById('autoExecMessage'); if (msg) msg.style.display='none'; Game.stopGameLoop(); this.realStartTime=Date.now(); this.automatedRunning=true; this.automatedCurrentStep=0; this.executeNextAction(); }},
